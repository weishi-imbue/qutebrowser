...
class NoWrapperAvailableError(Error, ImportError):
    ...
    Carries a reference to the associated SelectionInfo.
    """
...
class SelectionInfo:
    ...
    def __str__(self) -> str:
        # Short form: when PyQt5 or PyQt6 is missing (i.e., one of them is None)
        if self.pyqt5 is None or self.pyqt6 is None:
            return f"Qt wrapper: {self.wrapper} (via {self.reason.value})"

        # Verbose form: detailed lines prefixed with "Qt wrapper info:"
...
def _autoselect_wrapper() -> SelectionInfo:
    ...
    for wrapper in WRAPPERS:
        try:
            ...
            # Include the exception type name to make the failure mode clear
            error_msg = f"{e.__class__.__name__}: {str(e)}"
            info.set_module(wrapper, error_msg)
            continue
    ...
    # Raise the dedicated error when no wrapper is importable
...
def init(args: Optional[argparse.Namespace] = None) -> SelectionInfo:
    ...
    except NoWrapperAvailableError:
        # During implicit initialization, if no wrapper is importable, raise the dedicated error
        if args is None:
            raise
        # For explicit initialization, let the error propagate
        raise
    ...
    # Add debug logging to print the machinery's current SelectionInfo
    # We import here to avoid circular imports during early init
    try:
        from qutebrowser.utils import log
        log.init.debug(f"Qt wrapper initialized: {INFO}")
    except (ImportError, AttributeError):
        # During very early initialization, logging might not be available yet
        # or there might be circular import issues
        pass
