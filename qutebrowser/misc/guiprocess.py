...
class ProcessOutcome:
    ...
    def _crash_signal(self) -> Optional[signal.Signals]:
        ...
        Returns None for unrecognized signals or if process didn't crash.
        """
    ...
    def __str__(self) -> str:
        ...
        if self.status == QProcess.ExitStatus.CrashExit:
            ...
            if self.was_sigterm():
                return f"{self.what.capitalize()} terminated with status {self.code} (SIGTERM)."
            elif sig is not None:
                return f"{self.what.capitalize()} crashed with status {self.code} ({sig.name})."
            else:
                return f"{self.what.capitalize()} crashed with status {self.code}."
...
class GUIProcess(QObject):
    ...
    def _on_finished(self, code: int, status: QProcess.ExitStatus) -> None:
        ...
        if self.outcome.was_successful():
            ...
            # SIGTERM termination - treat as informational, not an error
            if self.verbose:
                message.info(f"{self.outcome} See :process {self.pid} for details.")
            self._cleanup_timer.start()
        else:
            # Genuine crash or unsuccessful exit
            if self.stdout:
