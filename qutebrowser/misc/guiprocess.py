        ...
        if self.status == QProcess.ExitStatus.CrashExit:
            if self.was_sigterm():
                ...
                try:
                    ...
                    # Unknown signal or not a signal code
...
class GUIProcess(QObject):
    ...
    def _on_finished(self, code: int, status: QProcess.ExitStatus) -> None:
        ...
        if self.outcome.was_successful():
            ...
            # Show error message based on process outcome and verbose setting
            show_error = True
            if self.outcome.was_sigterm() and not self.verbose:
                # Don't show error for SIGTERM unless verbose
                show_error = False
